        ...
        //  res.redirect('/users/error')
            ...
            con.query(sql3,(err,offer)=>{
              if(err){
                ...
                var foodSearch = "Select * from trainer"
...
router.post('/Ulogin',(req,res)=>{
  ...
  var sql="select * from product  where height=? and weight=?"
  con.query(sql,[pass,email],(err,result)=>{
    if(err){
      console.log(err);
      //res.redirect('/users/error')
    }
...
router.get('/buynow/:id/:mid',(req,res)=>{
  if(req.session.user){
  ...
  var user = req.session.user;
  res.render('user/payment',{homepage:true,mid,id,user})
  }else{
    res.render("user/userLogin",{homepage:true})  
  }
})
...
con.query(sql,data,(err,result)=>{
 ...
 router.get('/payemnts',(req,res)=>{
  var mid = req.session.user.height;
  var sql = "select *  from pay where mid = ?"
  con.query(sql,[mid],(err,result)=>{
    if(err){
      console.log(err)
    }else{
      var user = req.session.user;
      res.render('user/jobs',{fund:result,user})
    }
  })
}) 
